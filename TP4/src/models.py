...
class KMeans:
    ...
    def predict(self, X):
        if self.cluster_centers_ is None:
            raise ValueError("Ajustar el modelo con fit() antes de predecir.")
...
def build_kdtree(points, indices, depth=0):
    ...
    node = KDNode(
        ...
        index=sorted_idx[median],
        axis=axis,
        left=build_kdtree(points, sorted_idx[:median], depth + 1),
        right=build_kdtree(points, sorted_idx[median + 1:], depth + 1)
    )
    return node

def query_radius(node, points, target, eps, results):
    if node is None:
        return
    ...
    if diff <= eps:
        query_radius(node.left, points, target, eps, results)
    if diff >= -eps:
        query_radius(node.right, points, target, eps, results)
...
class DBSCAN:
    ...
    def fit(self, X):
        ...
        tree = build_kdtree(X, indices)
        for i in range(n_samples):
            if visited[i]:
                continue
            visited[i] = True
            neighbors = []
            query_radius(tree, X, X[i], self.eps, neighbors)
            neighbors = np.array(neighbors, dtype=int)
            if neighbors.size < self.min_samples:
                self.labels_[i] = -1
            else:
                self._expand_cluster(i, neighbors, cluster_id, visited, tree, X)
                cluster_id += 1
        self.n_clusters_ = cluster_id
        return self

    def _expand_cluster(self, idx, neighbors, cluster_id, visited, tree, X):
        ...
        while queue:
            ...
            if not visited[j]:
                visited[j] = True
                neigh_j = []
                query_radius(tree, X, X[j], self.eps, neigh_j)
                neigh_j = np.array(neigh_j, dtype=int)
...
class PCA:
    ...
    def inverse_transform(self, X_proj, clamp: bool = True, threshold: float = 128.0):
        """
        Reconstruye la aproximación en el espacio original.

        Parámetros:
        - X_proj: proyecciones (n_samples x n_components)
    ...
    def plot_explained_variance(self):
        ...
        plt.xlabel('Componente principal', fontsize = 17); plt.ylabel('Varianza individual', fontsize = 17)
        plt.title('Varianza explicada por componente', fontsize = 20); plt.xlim(1, len(ev))
        plt.xticks(fontsize=15); plt.yticks(fontsize=13)
        plt.tight_layout()

        # Zoom hasta comp. 100
        plt.figure(figsize=(8,6))
        plt.bar(idx, ev, alpha=0.7, color='teal')
        plt.xlabel('Componente principal', fontsize = 16); plt.ylabel('Varianza individual', fontsize = 16)
        plt.title('Varianza explicada (componentes 1–100)', fontsize = 18); plt.xlim(1, 100)
        plt.xticks(fontsize=13); plt.yticks(fontsize=13)
        plt.tight_layout()

        # Varianza acumulada
        thresholds = [0.80, 0.85, 0.90, 0.95]
        colors = ['mediumturquoise', 'darkred', 'navy', 'darkgreen']

        plt.figure(figsize=(10,6))
        plt.plot(idx, cv, marker='o', linewidth=1, color='crimson', label='Varianza acumulada', markersize=1.5)

        for thr, color in zip(thresholds, colors):
            comp_idx = idx[np.argmax(cv >= thr)]
        ...
        plt.title('Curva de varianza acumulada', fontsize = 18)
        plt.ylim(0,1.03)
        plt.xlim(1, len(cv))
...
class VAE(nn.Module):
    ...
    def plot_training_curves(self, train_losses, val_losses):
        ...
        plt.plot(train_losses, label='Train loss', color = "lightseagreen")
        plt.plot(val_losses,   label='Val loss', color = "lightcoral")
        plt.xlabel('Época', fontsize=16)
        plt.ylabel('Loss promedio', fontsize=16)
        plt.title('Curvas de entrenamiento para VAE', fontsize=18)
        plt.xticks(fontsize=13)
        plt.yticks(fontsize=13)
